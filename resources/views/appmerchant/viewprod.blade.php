...
?>
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: false,
ajax: "{{ url('/appmerchant/viewprod/' . request()->route('compid') . '/' .  request()->route('subcompid') . '/' .  request()->route('subsubcompid')) }}",
				columns: [
...
?>
		...
		<script type="text/javascript">		
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: false,
				ajax: "{{ url('/appmerchant/viewprod/' . request()->route('compid') . '/' .  request()->route('subcompid')) }}",
				columns: [
					{data: 'Delete1', name: 'Delete1', orderable: false, searchable: false},
					{data: 'id', name: 'id'},
					{data: 'name', name: 'name'},
	...
	?>
		<script type="text/javascript">		
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: false,
				ajax: "{{ url('/appmerchant/viewprod/' . request()->route('compid')) }}",
				columns: [
					{data: 'Delete1', name: 'Delete1', orderable: false, searchable: false},
					{data: 'id', name: 'id'},
					{data: 'name', name: 'name'},
