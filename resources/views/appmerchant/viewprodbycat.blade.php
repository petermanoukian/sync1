    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
				...
				<div class = 'floatleft'>
				<h1 class='text-dark fw-bolder fs-2 margintop7'> View Products (By Categories)</h1>
				</div> 
				<div class = 'floatleft'>
				...
				<a  href ='/appmerchant/addprodbycat/<?php echo $catid ; ?>/<?php echo $subcatid ; ?>/<?php echo $subsubcatid ; ?>'
				class='edit btn btn-primary btn-sm margintop7 paddtop6 bold left'>
				 Add Products </a>
				</div>
				</div>
                ...
                <div class="card-body">
					<form method = 'post' action = "/appmerchant/subsubprodbycat/deleteall">
...
?>
		<script type="text/javascript">		
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: false,
ajax: "{{ url('/appmerchant/viewprodbycat/' . request()->route('catid') . '/' .  request()->route('subcatid') . '/' .  request()->route('subsubcatid')) }}",
				columns: [
...
?>
		<script type="text/javascript">		
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: false,
				ajax: "{{ url('/appmerchant/viewprodbycat/' . request()->route('catid') . '/' .  request()->route('subcatid')) }}",
				columns: [
					{data: 'Delete1', name: 'Delete1', orderable: false, searchable: false},
					{data: 'id', name: 'id'},
					{data: 'name', name: 'name'},
	...
	?>
		<script type="text/javascript">		
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: false,
				ajax: "{{ url('/appmerchant/viewprodbycat/' . request()->route('catid')) }}",
				columns: [
					{data: 'Delete1', name: 'Delete1', orderable: false, searchable: false},
					{data: 'id', name: 'id'},
					{data: 'name', name: 'name'},
...
<script type="text/javascript">
  ...
  $(function () {
    ...
    var table = $('.data-table').DataTable({
        processing: true,
        serverSide: false,
        ajax: "{{ route('viewProdbycat.route') }}",
        columns: [
			{data: 'Delete1', name: 'Delete1', orderable: false, searchable: false},
            {data: 'id', name: 'id'},
            {data: 'name', name: 'name'},
